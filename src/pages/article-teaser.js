...
export default function ArticleTeaser({ article }) {
    ...
    return article ? (
        <a href={'articles/' + article.slug}
            ...
            className="w-full h-80 mt-10 shadow-xl px-10 py-5 bg-cover bg-no-repeat rounded flex flex-col items-stretch bg-right">
            <h2 className="text-lg font-semibold">{article.title}</h2>
            <p className="text-sm italic">{publicationDate.format('DD.MM.yyyy')}</p>
